...
import { Utensils, ListOrdered, Clipboard } from 'lucide-react';
...
const BottomNavigationBar: React.FC<BottomNavigationBarProps> = ({ activeTab, onTabChange, cartItemCount, themeColors }) => {
    const navItems = [
        ...
        { id: 'cart' as NavTab, label: 'الاوردر', icon: Clipboard, count: cartItemCount },
    ];
    return (
            ...
            <nav className="flex justify-around items-center h-16">
                {navItems.map((item) => (
                    <button key={item.id} onClick={() => onTabChange(item.id)}
                        ...
                        {item.id === 'cart' && (item?.count ?? 0) > 0 && <span className="absolute top-1.5 right-1/2 translate-x-[20px] bg-red-500 text-white text-[9px] font-bold w-4 h-4 rounded-full flex items-center justify-center">{item.count ?? 0 > 9 ? '' : item.count}</span>}
                    </button>
                ))}
            </nav>
        </footer>
