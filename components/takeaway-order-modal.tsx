// src/components/emenu/modals/TakeawayOrderModal.tsx
...
const TakeawayOrderModal: React.FC<TakeawayOrderModalProps> = ({ isOpen, onClose, onSubmit, themeColors }) => {
        ...
        <div className="fixed inset-0 bg-black/30 bg-opacity-50 flex items-center justify-center p-4 z-40" dir="rtl" onClick={onClose}>
            <div className="bg-white rounded-lg shadow-lg w-full max-w-md p-5 md:p-6" style={{ backgroundColor: themeColors?.background_color }} onClick={(e) => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg md:text-xl font-semibold" style={{color: themeColors?.text_color}}>طلب تيك أواي</h3>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <button type="submit" className="w-full py-2.5 md:py-3 rounded-lg font-semibold text-sm md:text-base" style={{backgroundColor: themeColors?.primary_color, color: themeColors?.text_color}}>
                        تأكيد الطلب
                    </button>
                </form>
            </div>
